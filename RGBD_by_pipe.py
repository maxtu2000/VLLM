...
class RGB_D:
    ...
    def statusCallback(self, msg):
        ...
        if status == 1:
            res = self.navigator.send_goal_through_client(0)
            if res:
        ...
        if status == 4:
            ...
            res = self.navigator.send_goal_through_client(1)
            if res:
    ...
    def isSameTarget(self,target):
        dx = target[0]- self.old_coordinate[0]
        dy = target[1]- self.old_coordinate[1]
    ...
    def calNavGoal(self, trash_loc, robot_loc, r):
        ...
        return trash_x, trash_y, quaternion
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.find_new_target:
                max_confidence=0.85
                result = self.model.track(img_color, persist=True, verbose=False)[0]
                ...
                for id in ids.keys():
                    if id in self.id_list:
                        ids[id][1] += self.id_list[id][1]
                self.id_list = ids

                max_d = None
                max_conf = 0
                for value in self.id_list.values():
                    d = value[0]
